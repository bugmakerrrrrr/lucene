...
/**
 * {@link CachedBytesRefHash} is a special purpose hash-map like data-structure optimized for {@link
 * BytesRef} instances. CachedBytesRefHash maintains mappings of byte arrays to ids
 * (Map&lt;BytesRef,int&gt;) storing the hashed bytes efficiently in continuous storage. The mapping
 * to the id is encapsulated inside {@link CachedBytesRefHash} and is guaranteed to be increased for
 * each added {@link BytesRef}.
 *
 ...
 * @lucene.internal
 */
public final class CachedBytesRefHash implements Accountable {
  private static final long BASE_RAM_BYTES =
      RamUsageEstimator.shallowSizeOfInstance(CachedBytesRefHash.class)
          +
          // size of Counter
  ...
  /**
   * Creates a new {@link CachedBytesRefHash} with a {@link ByteBlockPool} using a {@link
   * DirectAllocator}.
   */
  public CachedBytesRefHash() {
    this(new ByteBlockPool(new DirectAllocator()));
  }

  /** Creates a new {@link CachedBytesRefHash} */
  public CachedBytesRefHash(ByteBlockPool pool) {
    this(pool, DEFAULT_CAPACITY, new DirectBytesStartArray(DEFAULT_CAPACITY));
  }

  /** Creates a new {@link CachedBytesRefHash} */
  public CachedBytesRefHash(ByteBlockPool pool, int capacity, BytesStartArray bytesStartArray) {
    hashSize = capacity;
  ...
  /**
   * Returns the number of {@link BytesRef} values in this {@link CachedBytesRefHash}.
   *
   * @return the number of {@link BytesRef} values in this {@link CachedBytesRefHash}.
   */
  public int size() {
    return count;
  }
   ...
   * this {@link CachedBytesRefHash} instance.
   *
   * @lucene.internal
   */
  public int[] compact() {
  ...
  /** Closes the CachedBytesRefHash and releases all internally used memory */
  public void close() {
    clear(true);
    ids = null;
    hashcodes = null;
  ...
  private int findHash(BytesRef bytes) {
    ...
    // (see LUCENE-5604):
    while (e != -1
        && (hashcodes[hashPos] != hashcode || pool.equals(bytesStart[e], bytes) == false)) {
      code++;
   ...
   * instead reference the byte[] term already stored by the postings CachedBytesRefHash. See
  ...
  private void rehash(final int newSize, boolean hashOnData) {
    ...
    final int[] newIds = new int[newSize];
    ...
    for (int i = 0; i < hashSize; i++) {
      ...
      if (e0 != -1) {
        ...
        int code;
        if (hashOnData) {
          hashcode = code = hashcodes[i];
          assert hashcode == pool.hash(bytesStart[e0]);
        } else {
          hashcode = 0;
          code = bytesStart[e0];
        }
        ...
        // Conflict; use linear probe to find an open slot
        // (see LUCENE-5604):
        while (newIds[hashPos] != -1) {
          code++;
          hashPos = code & newMask;
        }

        newIds[hashPos] = e0;
        newHashcodes[hashPos] = hashcode;
      }
    }
  ...
  /**
   * reinitializes the {@link CachedBytesRefHash} after a previous {@link #clear()} call. If {@link
  ...
  public long ramBytesUsed() {
    return BASE_RAM_BYTES
        ...
        + RamUsageEstimator.sizeOfObject(hashcodes);
  }

  /**
   * Thrown if a {@link BytesRef} exceeds the {@link CachedBytesRefHash} limit of {@link
  ...
  public abstract static class BytesStartArray {
     ...
     * The {@link CachedBytesRefHash} uses this reference to track it memory usage
     *
